    ...
    <section className="mx-auto grid max-w-7xl gap-8 px-5 pb-20 lg:grid-cols-[1fr_360px]">
      <div className="grid gap-8 md:gap-6">
        {articles.map((article, index) => (
          <div key={article.slug} className="grid gap-8 md:gap-6">
            <Link href={`/articles/${article.slug}`} className="group glass-card grid gap-5 overflow-hidden rounded-3xl p-4 transition hover:-translate-y-1 md:grid-cols-[320px_1fr]">
            ...
            {(index + 1) % 4 === 0 ? <SponsoredCard /> : null}
          </div>
        ))}
      </div>
      ...
      <aside className="space-y-6 lg:sticky lg:top-28 lg:self-start">
        ...
        <SidebarAds tools={tools} categories={categories} />
      </aside>
    </section>
  );
}
